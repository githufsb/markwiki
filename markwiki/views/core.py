...
def render_wiki_appendor(page):
    '''Render the wiki editor with content from the provided wiki page.
    Assumes a valid wiki page.'''
    try:
        return render_template('append.html', page_path=page.page_path,
                               wiki_content=page.content)
    except IOError:
        abort(500)
...
@app.route('/append/')
@app.route('/append/<path:page_path>')
@login_required
def append(page_path=None):
    ...
    try:
        ...
        # Proceed if the wiki exists.
        if page.exists:
            return render_wiki_appendor(page)
        else:
            # Get the user going with this new page.
            return redirect(url_for('create', page_path=page_path))
    except ValidationError as verror:
...
@app.route('/append_wiki', methods=['POST'])
@login_required
def append_wiki():
    '''Update a wiki page.'''
    # If the path changed, then this is now a new page.
